...
#throw head words in as normal words, assume BPE units for gloss words
#still strip heads from glosses
...
def pad_sequence(sequence, max_seq_len):
  ...
  # Sentence too short, so pad.
    ...
    padded = np.append(sequence, ([PAD_ID] * padding_required))
  return padded
...
def data_to_token_ids_BPE(data_path, target_path, vocabulary_path, max_seq_len,
                      tokenizer=None, normalize_digits=True):
  """Tokenize data file and turn into token-ids using given vocabulary file.
      ...
      with tf.gfile.GFile(target_path + ".gloss", mode="w") as glosses_file:
        with tf.gfile.GFile(target_path + ".head", mode="w") as heads_file:
          ...
          for line in data_file:
            ...
            # Write out the head ids, one head per line.
            heads_file.write(str(token_ids[0]) + "\n")
            # Cleaning has already taken place for bpe data
#            clean_gloss = [w for w in token_ids[1:] if w != token_ids[0]]
            gloss = [w for w in token_ids[1:]]
            # Padding also no longer required as it takes place during training.
            glosses_ids = gloss
            # Write out the glosses as ids, one gloss per line.
            glosses_file.write(" ".join([str(t) for t in glosses_ids]) + "\n")
...
def prepare_dict_data(data_dir, train_file, dev_file, test_file,
  ...
  test_ids = os.path.join(data_dir, "test.definitions.ids%d" % vocabulary_size)
  data_to_token_ids_BPE(
      train_path, train_ids, vocab_path_stem, max_seq_len, tokenizer)
  data_to_token_ids_BPE(
      dev_path, dev_ids, vocab_path_stem, max_seq_len, tokenizer)
  data_to_token_ids_BPE(
      test_path, test_ids, vocab_path_stem, max_seq_len, tokenizer)
